        ...
        if DRAW_LINES:
            try:
                pygame.draw.line(win, (255, 0, 0),
                                 (mario.x + mario.img.get_width() / 2, mario.y + mario.img.get_height() / 2),
                                 (pipes[pipe_ind].x + pipes[pipe_ind].get_width() / 2, pipes[pipe_ind].bottom), 5)
            except:
                pass
        mario.draw(win)
...
def eval_genomes(genomes, config):
    ...
    while run and len(marios) > 0:
        ...
        for x, mario in enumerate(marios):
            ge[x].fitness += .10
            output = nets[marios.index(mario)].activate(
                (mario.y, abs(mario.x - pipes[pipe_ind].x), abs(mario.height - pipes[pipe_ind].bottom)))
            # print("Mario #{}: Dist {} :Mario/pipe Dist {}: Mario y: {}".format(marios.index(mario), mario.y,
            #                                                                    abs(mario.x - pipes[pipe_ind].x),
            #                                                                    abs(mario.y - pipes[pipe_ind].bottom)))
